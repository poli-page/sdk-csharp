...
`languages.js` carries a local entry for `csharp` (display name `C#`, example
extension `.cs`, package kind `nuget`). The upstream preset does not yet
include C# in its `LanguageId` union — when it does, sync the file back from
upstream and keep the entry there (or in a single shared source). Until then,
edits to the upstream `languages.js` need this entry re-applied on sync.
